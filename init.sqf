...
//----------------------------Presets---------------------------------------------------------
mccPresetsVehicle = [
					 [localize "STR_MCC_Zeus_Fuel", '_this setfuel 0;']
					,[localize "STR_MCC_Zeus_Ammo", '_this setvehicleammo 0;']
					,[localize "STR_MCC_Zeus_Cargo", 'clearMagazineCargoGlobal _this; clearWeaponCargoGlobal _this; clearItemCargoGlobal _this;']
					,[localize "STR_MCC_Zeus_Locked", '_this setVehicleLock "LOCKED";']
					,[localize "STR_MCC_Zeus_Cargo_Local", 'clearMagazineCargo _this; clearWeaponCargo _this; clearItemCargo _this;']
					,[localize "STR_MCC_Zeus_UAV", 'if (isServer) then {createVehicleCrew _this;group _this setvariable ["MCC_canbecontrolled",true,true];};']
					,[localize "STR_MCC_Zeus_Units", 'if (isServer) then {[_this] call MCC_fnc_populateVehicle};']
					,[localize "STR_MCC_Zeus_ECM", 'if (isServer) then {_this setvariable ["MCC_ECM",true,true]};']
					,[localize "STR_MCC_Zeus_FOB", '_this addAction [localize "STR_Create_Fob", "'+MCC_path+'mcc\roleSelection\scripts\createFOB.sqf",[],6,false, false,"teamSwitch","(driver vehicle _target == _this) && (speed (vehicle _target) == 0)"];']
					,[localize "STR_MCC_Zeus_Sim", '_this enableSimulation false;']
					,[localize "STR_MCC_Zeus_Control", '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,[localize "STR_MCC_Zeus_Recruit", '_this addAction [format [localize "STR_Recruit", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,[localize "STR_MCC_Zeus_Join", '[_this] join (group player);']
					,[localize "STR_MCC_Zeus_Enemy", '_this addrating -2001;']
					,[localize "STR_MCC_Zeus_Moile_West", '[west,_this] call BIS_fnc_addRespawnPosition;']
					,[localize "STR_MCC_Zeus_Moile_East", '[east,_this] call BIS_fnc_addRespawnPosition;']
					,[localize "STR_MCC_Zeus_Moile_Guer", '[resistance,_this] call BIS_fnc_addRespawnPosition;']
					,['', '']
					,[localize "STR_MCC_Zeus_Arti",'']
					,[localize "STR_MCC_Zeus_Arti_Cannon", '[0,_this] spawn MCC_fnc_amb_Art;']
					,[localize "STR_MCC_Zeus_Arti_Rocket", '[1,_this] spawn MCC_fnc_amb_Art;']
					,[localize "STR_MCC_Zeus_AA_Cannon", '[2,_this] spawn MCC_fnc_amb_Art;']
					,[localize "STR_MCC_Zeus_AA_Light", '[3,_this] spawn MCC_fnc_amb_Art;']
					,[localize "STR_MCC_Zeus_General",'']
					,[localize "STR_MCC_Zeus_Destroy_Veh", '_this setdamage 1;']
					,[localize "STR_MCC_Zeus_Flip_Veh", '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[localize "STR_MCC_Zeus_Arsenal", 'if (isServer) then {["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal};']
					,[localize "STR_MCC_Zeus_Satchels", '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,[localize "STR_MCC_Zeus_God", '_this allowDamage false;']
					,[localize "STR_MCC_Zeus_Carrier", '[_this,"Aircraft Carrier Console","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
					,[localize "STR_MCC_Zeus_Effects",'']
					,[localize "STR_MCC_Zeus_Sand",'[_this] call BIS_fnc_sandstorm;']
					,[localize "STR_MCC_Zeus_Flies",'[getposatl _this] call BIS_fnc_flies;']
					,[localize "STR_MCC_Zeus_Smoke",'if (isServer) then {_effect = "test_EmptyObjectForSmoke" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,[localize "STR_MCC_Zeus_Fire",'if (isServer) then {_effect = "test_EmptyObjectForFireBig" createVehicle (getpos _this);_effect setpos (getpos _this)};']
					,['', '']
					,[localize "STR_MCC_Zeus_Misk",'']
					,[localize "STR_MCC_Zeus_Marker", '_this execVM "'+MCC_path+'mcc\general_scripts\create_local_marker.sqf";']
					];

mccPresetsUnits = [
					 [localize "STR_MCC_Zeus_Recruit", '_this addAction [format ["Recruit %1", name _this], "'+MCC_path+'mcc\general_scripts\hostages\hostage.sqf",[2],6,false,true];']
					,[localize "STR_MCC_Zeus_Hostage", '_this execVM "'+MCC_path+'mcc\general_scripts\hostages\create_hostage.sqf";']
					,[localize "STR_MCC_Zeus_Join", '[_this] join (group player);']
					,[localize "STR_MCC_Zeus_Enemy", '_this addrating -2001;']
					,[localize "STR_MCC_Zeus_Stand", '_this setUnitPos "UP";']
					,[localize "STR_MCC_Zeus_Kneel", '_this setUnitPos "Middle";']
					,[localize "STR_MCC_Zeus_Prone", '_this setUnitPos "DOWN";']
					,[localize "STR_MCC_Zeus_Remove_Weap", 'removeAllWeapons _this;']
					,[localize "STR_MCC_Zeus_Remove_Items", 'removeAllItems _this;']
					,[localize "STR_MCC_Zeus_Unconsciou", '[_this,_this] spawn MCC_fnc_unconscious']
					,[localize "STR_MCC_Zeus_Control", '(group _this) setvariable ["MCC_canbecontrolled",true,true];']
					,[localize "STR_MCC_Zeus_God", '_this allowDamage false;']
					,['', '']
					,[localize "STR_MCC_Zeus_General",'']
					,[localize "STR_MCC_Zeus_Kill_Unit", '_this setdamage 1;']
					,[localize "STR_MCC_Zeus_Flip_Unit", '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[localize "STR_MCC_Zeus_Satchels", '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,['', '']
					,[localize "STR_MCC_Zeus_Effects",'']
					,[localize "STR_MCC_Zeus_Sand",'[_this] call BIS_fnc_sandstorm;']
...
mccPresetsObjects = [
					 [localize "STR_MCC_Zeus_Pick",'_this call MCC_fnc_pickItem;']
					,[localize "STR_MCC_Zeus_Sim",'_this enableSimulation false;']
					,[localize "STR_MCC_Zeus_Destroy_Obj", '_this setdamage 1;']
					,[localize "STR_MCC_Zeus_Flip_Obj", '[_this ,0, 90] call bis_fnc_setpitchbank;']
					,[localize "STR_MCC_Zeus_Arsenal", '["AmmoboxInit",[_this,true]] call BIS_fnc_arsenal']
					,[localize "STR_MCC_Zeus_Satchels", '_this addEventHandler ["handledamage", {if ((_this select 4) in ["SatchelCharge_Remote_Ammo","DemoCharge_Remote_Ammo"]) then {(_this select 0) setdamage 1;(_this select 3) addRating 1500} else {0}}];']
					,[localize "STR_MCC_Zeus_God", '_this allowDamage false;']
					,[localize "STR_MCC_Zeus_Carrier", '[_this,"Aircraft Carrier Console","","","(alive _target) && (_target distance _this < 5)","(alive _target) && (_target distance _this < 5)",{},{},{[] spawn MCC_fnc_LHDspawnMenuInit},{},[],3,10,false,false] call bis_fnc_holdActionAdd;']
					,['', '']
