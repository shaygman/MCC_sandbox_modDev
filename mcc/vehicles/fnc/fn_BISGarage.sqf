/*======================================================MCC_fnc_BISGarage===============================================================================================
	Replace the BI Garage function for zeus all credits go to Zeus

	missionNamespace setVariable ["BIS_fnc_garage_center",_this];
	missionNamespace setVariable ["BIS_fnc_arsenal_center",_this];
	uiNamespace setVariable ["bis_fnc_garage_defaultClass",typeof _this];
	["InitGUI",[]] spawn bis_fnc_arsenal;
	[] spawn MCC_fnc_BISGarage
...
switch _mode do {
	...
	case "Open": {
		...
		with uinamespace do {
			...
			if !(isnull finddisplay 312) then {_displayMission = finddisplay 312;_displayClass = "RscDisplayGarageMCC";};
	...
	case "Init": {
		...
		INITTYPES
		["InitGUI",[_display,"bis_fnc_garage"]] spawn bis_fnc_arsenal;
		["Preload"] call MCC_fnc_BISGarage;
		["ListAdd",[_display]] call MCC_fnc_BISGarage;
		//["ListSelectCurrent",[_display]] call BIS_fnc_garage;
		if (BIS_fnc_garage_centerType == "") then {["buttonRandom",[_display]] call MCC_fnc_BISGarage;};
...
*/
	...
	case "ListAdd": {
		...
		if (true) then {
			...
			["ShowItemStats",[_cfg]] call MCC_fnc_BISGarage;
...
systemChat "TabSelectLeft";
		{
			...
			if (_active) then {
				...
				['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _idc),_idc]] call MCC_fnc_BISGarage;
			} else {
		...
		['TabSelectRight',[_display,IDC_RSCDISPLAYGARAGE_TAB_SUBCREW]] call MCC_fnc_BISGarage;
	};
...
*/
		...
		if (isclass _cfg) then {
			...
			["ShowItemStats",[_cfgStats]] call MCC_fnc_BISGarage;
		};
	};
			...
			if (_categoryIndex >= 0) then {
				...
				if (_nameID >= 0) then {
					...
					[_center,_name,_categoryIndex,_display] spawn { //--- Must be spawned, animations cannot be applied immediately
						...
						if (is3DEN) then {
							['SelectItem',[_display,_display displayctrl (IDC_RSCDISPLAYARSENAL_LIST + _categoryIndex),_categoryIndex]] call MCC_fnc_BISGarage;
						} else {
