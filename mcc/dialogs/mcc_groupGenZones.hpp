class MCC_zoneControls: MCC_RscControlsGroup
{
	...
	class Controls
	{
		...
		{
			idc = -1;
			text = $STR_Zone_Create;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
			tooltip = $STR_Zone_Create_Tip;
		...
		class MCC_gaiaBehaviorButton: MCC_RscButton
		{
			...
			text = $STR_Zone_Give_Gaia; //--- ToDo: Localize;
			x =0.131771 * safezoneW;
			y = 0.0989618 * safezoneH;
			w = 0.0572917 * safezoneW;
			h = 0.0219914 * safezoneH;
			tooltip = $STR_Zone_Give_Gaia_Tip; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
		};
		...
		class MCC_giveToPlayer: MCC_RscButton
		{
			...
			text = $STR_Zone_Give_Player; //--- ToDo: Localize;
			x =0.131771 * safezoneW;
			y = 0.0659748 * safezoneH;
			w = 0.0572917 * safezoneW;
			h = 0.0219914 * safezoneH;
			tooltip = $STR_Give_Player_Tip; //--- ToDo: Localize;
			sizeEx = "(((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * 0.7)";
		};
	};
};